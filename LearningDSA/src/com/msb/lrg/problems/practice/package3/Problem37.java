package com.msb.lrg.problems.practice.package3;

/*
Maximum Product Subarray :: LeetCode (152 Medium)
	Given an integer array nums, find a subarray that has the largest product, and 
	return the product.
	The test cases are generated so that the answer will fit in a 32-bit integer.
	Note that the product of an array with a single element is the value of that element.
	Example 1:
		Input: nums = [2,3,-2,4]
		Output: 6
		Explanation: [2,3] has the largest product 6.
	Example 2:
		Input: nums = [-2,0,-1]
		Output: 0
		Explanation: The result cannot be 2, because [-2,-1] is not a subarray.
	Constraints:
		1 <= nums.length <= 2 * 104
		-10 <= nums[i] <= 10
		The product of any subarray of nums is guaranteed to fit in a 32-bit integer.

 */
public class Problem37 {

	public static void main(String[] args) {
//		int[] nums = {2,3,-2,4};
//		int[] nums = {-2};
		int[] nums = {-4,-3,-2};
		int res = maxProductBrute(nums);
		System.out.println("Result Brute:: " + res);
		res = maxProductPrefixSuffix(nums);
		System.out.println("Result Prefix/Suffix :: " + res);
		res = maxProductDP(nums);
		System.out.println("Result DP :: " + res);
	}

	public static int maxProductBrute(int[] nums) {
		int max = Integer.MIN_VALUE;
		int n = nums.length;
		
		for(int i = 0; i < n; i++) {
			int prod = 1;
			for(int j = i; j < n; j++) {
				prod *= nums[j];
				max = Math.max(max, prod);
			}
		}
		return max;
	}

    public static int maxProductPrefixSuffix(int[] nums) {
		int max = Integer.MIN_VALUE;
		int n = nums.length;
		int prefix = 1, suffix = 1;
		
		for(int i = 0; i < n; i++) {
			prefix = (prefix == 0 ? 1 : prefix) * nums[i];
			suffix = (suffix == 0 ? 1 : suffix) * nums[n -1 - i];
			max = Math.max(max, Math.max(suffix, prefix));
		}
		
		return max;
    }
	
    public static int maxProductDP(int[] nums) {
		int max = nums[0];
		int maxEnding = nums[0];
		int minEnding = nums[0];
		int n = nums.length;
		
		for(int i = 1; i < n; i++) {	
			int curr = nums[i];
			int tempMax = Math.max(curr, Math.max(curr * minEnding, curr * maxEnding));
			minEnding = Math.min(curr, Math.min(curr * minEnding, curr * maxEnding));
			maxEnding = tempMax;
			max = Math.max(max, maxEnding);
		}
        
		return max;
    }
    
}
